...
//!
//! [`open_depth_first_events`] walks the same open space for a *fold*. A
//! search can only stop; a fold computes a node's answer from its subtrees'
//! answers, which needs the unwind — so the walk reports
