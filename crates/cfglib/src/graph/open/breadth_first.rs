...
mod tests {
    ...
    fn seen(event: OpenBfsEvent<'_, u32>) -> Seen {
        match event {
            OpenBfsEvent::Discover(&node, depth) => Seen::Discover(node, depth),
            OpenBfsEvent::Refused(&node, depth) => Seen::Refused(node, depth),
        }
    }
    ...
    fn skipping_and_depth_bounds_prevent_successor_discovery() {
        ...
        assert_eq!(outcome, None);
        assert_eq!(events, vec![Seen::Discover(0, 0), Seen::Discover(1, 1)]);
