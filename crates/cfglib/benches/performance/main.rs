...
use fixtures::{
    ...
    build_if_else_chain, build_two_case_switch_chain, empty_chain_cfg, fixture_u32,
    high_fan_in_cfg, independent_constants, irreducible_cfg, linear_cfg, linear_constants,
    many_exit_cfg, multi_latch_graph, phi_storm_cfg, reverse_id_chain_graph,
    weighted_high_fan_out_cfg, weighted_irreducible_cfg,
};
...
fn main() {
    ...
    let constant_ssa = SsaForm::compute(&constants, &constant_dominators);
    ...
    let mut suite = BenchmarkSuite::new(&filter, target);
    macro_rules! bench {
        ($name:literal, $operation:expr, $oracle:expr) => {{
            benchmark_case!(suite, $name, $operation, $oracle);
        }};
    }
    ...
    bench!(
        ...
        || IntervalAnalysis::compute(&cfg),
        |result: &IntervalAnalysis| assert_cfg_intervals(result, &cfg)
    );
    bench!(
        "directed_interval_reverse_id_chain",
        || IntervalAnalysis::compute(&Rooted::new(&reverse_id_chain, reverse_id_root)),
    ...
    bench!(
        ...
        || {
            solve_node_problem(&graph, &Reachability)
                .expect("an unbounded solve cannot exceed its step limit")
        },
    ...
    bench!(
        "cfg_fixpoint_bool",
        || {
            solve_problem(&cfg, &CfgReachability)
                .expect("an unbounded solve cannot exceed its step limit")
        },
        |result: &Facts<bool>| assert_bool_cfg_facts(result, NODE_COUNT)
    );
    bench!(
        "cfg_fixpoint_wide",
        || {
            solve_problem(&wide_cfg, &WideCfgFact)
                .expect("an unbounded solve cannot exceed its step limit")
        },
        |result: &Facts<Vec<u64>>| assert_wide_cfg_facts(result, 1_024)
    );
    bench!(
        "cfg_sccp_independent_constants",
        || SccpAnalysis::compute(&constants, &constant_ssa),
        |result: &SccpAnalysis<u32, u64>| {
    ...
    bench!(
        ...
        |result: &Facts<BTreeMap<u32, ConstValue<u64>>>| {
            assert!(result.fact_in(constants.entry()).is_empty());
            assert_eq!(result.fact_out(constants.entry()).len(), 1_024);
            for variable in 0..1_024_u32 {
                assert_eq!(
                    result.fact_out(constants.entry()).get(&variable),
                    Some(&ConstValue::Const(u64::from(variable)))
                );
            }
        }
    ...
    api_cases::register(&mut suite);
    suite.finish(api_cases::PUBLIC_API_FUNCTIONS);
}
